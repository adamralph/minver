...
public static class OutputVariables
{
    ...
    public static async Task AreSet()
    {
        ...
        var envVars = ("MinVerVersionOverride".ToAltCase(), "2.3.4.5-alpha-x.5+build.6");

        // act
        var (_, standardOutput, _) = await Sdk.BuildProject(path, envVars: envVars);

        // assert
        Assert.Contains("MinVer: [output] MinVerVersion=2.3.4.5-alpha-x.5+build.6", standardOutput, StringComparison.Ordinal);
        ...
        Assert.Contains("MinVer: [output] MinVerRevision=5", standardOutput, StringComparison.Ordinal);
        ...
        Assert.Contains("MinVer: [output] FileVersion=2.3.4.5", standardOutput, StringComparison.Ordinal);
        Assert.Contains("MinVer: [output] InformationalVersion=2.3.4.5-alpha-x.5+build.6", standardOutput, StringComparison.Ordinal);
        Assert.Contains("MinVer: [output] PackageVersion=2.3.4.5-alpha-x.5", standardOutput, StringComparison.Ordinal);
        Assert.Contains("MinVer: [output] Version=2.3.4.5-alpha-x.5", standardOutput, StringComparison.Ordinal);
    }
}
